    ...
    class Interactive_Onboarding_Tool_Activator
    {
        ...
        public function get_available_clinician_data()
        {
            ...
            foreach ($common_data as $c_data)
            {
                ...
                // if($c_data->ID == 6247) {
                // echo $c_data->ID."id";
                // print_r($av_consultation);
                // echo "========";
                // print_r($db_consultation);
                // echo "========";
                // print_r($av_location_area);
                // echo "========";
                // print_r($db_location_area);
                // echo "========";
                // print_r($av_gender_arr);
                // echo "========";
                // print_r($db_gender);
                // echo "========";
                // print_r($av_who_treatment);
                // echo "========";
                // print_r($db_who_treatment);
                // echo "========";
                // print_r($av_what_treatment);
                // echo "========";
                // print_r($db_what_treatment);
                // echo "========";
                // print_r($av_language);
                // echo "========";
                // print_r($db_language);
                ...
                // }
                    ...
                    {
                        if ( in_array($av_consultation, $db_consultation) && in_array($db_gender, $av_gender_arr)  && in_array($av_what_treatment, $db_what_treatment)  &&  in_array($av_language, $db_language)  && in_array($av_location_area, $db_location_area) && in_array($av_who_treatment, $db_who_treatment) && in_array($c_data->ID, $available_clinicians))
                        {
                    ...
                    else
                    {
                        ...
                        if (in_array($av_consultation, $db_consultation) && in_array($db_gender, $av_gender_arr)  && in_array($av_what_treatment, $db_what_treatment)  &&  in_array($av_language, $db_language)  && in_array($av_location_area, $db_location_area) && in_array($av_who_treatment, $db_who_treatment))
                        {
                    ...
                    if (in_array($av_consultation , $db_consultation)){ 
                        $matches_results_get[$c_data->ID]['consultation'] = 1;
                        $i++;
                    }

                    if (in_array($db_gender, $av_gender_arr)){
                        $matches_results_get[$c_data->ID]['gender'] = 1;
                        $i++;
                    }
                    if (in_array($av_who_treatment, $db_who_treatment)){
                        $matches_results_get[$c_data->ID]['av_who_treatment'] = 1;
                        $i++;
                    }
                    if (in_array($av_what_treatment, $db_what_treatment)){
                        $matches_results_get[$c_data->ID]['av_what_treatment'] = 1;
                        $i++;
                    }
                 
                    if (in_array($av_location_area, $db_location_area)){
                        $matches_results_get[$c_data->ID]['location'] = 1;
                        $i++;
                    }


                    if (in_array($av_language, $db_language)){
            ...
            foreach ($exact_matches as $exact_key => $exact_match){
                ...
                $clinican_exact_book_now_url = '';
                switch(trim($av_consultation)){
                    case 'In-person (a face-to-face consultation)':
                        $clinican_exact_book_now_url = get_post_meta($exact_match,'f2f_book_now_url', true); 
                    break;
                    case 'Telehealth (a video consultation)':
                        $clinican_exact_book_now_url = get_post_meta($exact_match,'telehealth_video_consultation_book_now_url', true);
                    break;
                    case 'Telehealth (a phone consultation)':
                        $clinican_exact_book_now_url = get_post_meta($exact_match,'telehealth_phone_consultation_book_now_url', true);
                    break;
                    default :
                    $clinican_exact_book_now_url = get_post_meta($exact_match,'f2f_book_now_url', true);  
            ...
            foreach ($close_matches as $close_key => $close_match){
                ...
                $clinican_close_book_now_url = '';
                switch(trim($av_consultation)){
                    case 'In-person (a face-to-face consultation)':
                        $clinican_close_book_now_url = get_post_meta($close_key,'f2f_book_now_url', true); 
                    break;
                    case 'Telehealth (a video consultation)':
                        $clinican_close_book_now_url = get_post_meta($close_key,'telehealth_video_consultation_book_now_url', true);
                    break;
                    case 'Telehealth (a phone consultation)':
                        $clinican_close_book_now_url = get_post_meta($close_key,'telehealth_phone_consultation_book_now_url', true);
                    break;
                    default :
                    $clinican_close_book_now_url = get_post_meta($close_key,'f2f_book_now_url', true);  

                }
                //$clinican_book_now_url = get_post_meta($close_key,'book_now_url', true);
                $close_matches_html .= '<div class="col-xs-12 col-sm-3"><div class="pyschologist-block"><a href="' . $clinican_link . '" title="' . $clinican_title . '"><img src="' . $clinican_img[0] . '" data-spai="1" alt="' . $clinican_title . ', Counsellor" class="portrait thumbnail socially-awkward-large" width="133" height="160" itemprop="image" data-spai-upd="138"></a><h2 class="entry-title" itemprop="headline"><a href="' . $clinican_link . '" rel="bookmark" itemprop="url"><div class="read-plus"></div>' . $clinican_title . '</a><p>' . implode(",", array_unique($location_close_data)) . '</p><p>' . implode(",", $cat_data) . '</p><a class="book_now_url" href="'.$clinican_close_book_now_url.'">BOOK NOW</a></h2></div></div>';
            }
            $close_matches_html .= "</div></div>";

            }
            else{
                $close_matches_html .= "<div class='notify_matches'>Sorry, We didn't find Close matches..you can checkout other matches here </div>";
            }

            if (!empty($others_consideration)){
            $others_consideration_html .= "<div class='other_consideratiob_section'> <h3> Others consider: </h3><div class='others_consider_col'>";
            ...
            foreach ($others_consideration as $other_key => $others_consider) {
                ...
                $clinican_others_book_now_url = '';
                switch(trim($av_consultation)){
                    case 'In-person (a face-to-face consultation)':
                        $clinican_others_book_now_url = get_post_meta($other_key,'f2f_book_now_url', true); 
                    break;
                    case 'Telehealth (a video consultation)':
                        $clinican_others_book_now_url = get_post_meta($other_key,'telehealth_video_consultation_book_now_url', true);
                    break;
                    case 'Telehealth (a phone consultation)':
                        $clinican_others_book_now_url = get_post_meta($other_key,'telehealth_phone_consultation_book_now_url', true);
                    break;
                    default :
                    $clinican_others_book_now_url = get_post_meta($other_key,'f2f_book_now_url', true);  

                }
                // echo $clinican_others_book_now_url;
               // $clinican_book_now_url = get_post_meta($other_key,'book_now_url', true);
                $others_consideration_html .= '<div class="col-xs-12 col-sm-3"><div class="pyschologist-block"><a href="' . $clinican_link . '" title="' . $clinican_title . '"><img src="' . $clinican_img[0] . '" data-spai="1" alt="' . $clinican_title . ', Counsellor" class="portrait thumbnail socially-awkward-large" width="133" height="160" itemprop="image" data-spai-upd="138"></a><h2 class="entry-title" itemprop="headline"><a href="' . $clinican_link . '" rel="bookmark" itemprop="url"><div class="read-plus"></div>' . $clinican_title . '</a><p></p>' . implode(",", $location_others_data) . '<p>' . implode(",", $cat_data) . '</p><a class="book_now_url" href="'.$clinican_others_book_now_url.'">BOOK NOW</a></h2></div></div>';
            }
            $others_consideration_html .= "</div></div>";

         }
        else{
            $others_consideration_html .= "<div class='notify_matches'></div>";
        }


            $response = array();

            $headline = '<div class="headline"><h2> Here are your recommneded NVP clinicians </h2><p> Based on your preferences, we have found these matching profiles.To make a booking, click on the clinician\'s  <b>Book Now</b> button. </p></div>';
